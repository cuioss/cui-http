...
[source,bash]
----
# Smoke benchmark run, benchmarking module only — not measurement-grade.
# The `benchmark` profile enables execution; `smoke` cuts the iteration and warmup counts
# while lengthening each measurement window, so total wall time drops.
python3 .plan/execute-script.py plan-marshall:build-maven:maven run \
    --command-args "verify -Pbenchmark,smoke -pl cui-http-benchmarking -am"
----
