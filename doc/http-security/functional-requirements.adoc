...
* The system must support configuration of validation parameters:
  ...
  ** Attack-pattern block lists: the sensitive-path block list (`blockedPathPatterns`, matched as a
     whole `/`-delimited path segment) and the parameter-name block list
     (`blockedParameterNames`, matched by exact equality), alongside the existing header-name and
