...
`LengthValidationStage` → `CharacterValidationStage` → `AllowBlockListStage`, all under `HEADER_VALUE`.
`DecodingStage`, `NormalizationStage`, and `PatternMatchingStage` are deliberately absent, for the same reasons as on `HTTPHeaderValidationPipeline`.
