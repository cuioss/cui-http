...
`failOnSuspiciousPatterns=false`). Use `SecurityConfiguration.strict()` for the maximum-security
posture with "strict validation / maximum checks" described above. See
...
* NOTE: Runtime-configurable attack patterns are *not implemented*. The attack patterns in
  `PatternMatchingStage` / `SecurityDefaults` are `static final` compile-time constants, and
  `SecurityConfiguration` exposes *no* pattern knob. Updating patterns therefore requires a
  code change and a new release.
* `SecurityConfiguration` does expose configurable *policy* toggles (length limits,
  encoding/character policy, allow/block lists), but not the attack-pattern set itself.
