...
Path canonicalization is split across two stages:

* link:../../../cui-http-core/src/main/java/de/cuioss/http/security/validation/DecodingStage.java[DecodingStage] performs Unicode canonicalization — *NFKC* for URL paths and *NFC* for parameter values (normalize-and-continue). It does *not* use NFD or NFKD.
* link:../../../cui-http-core/src/main/java/de/cuioss/http/security/validation/NormalizationStage.java[NormalizationStage] performs RFC 3986 Section 5.2.4 dot-segment resolution (`.`/`..`); it contains no `java.text.Normalizer` usage.
...
|Null Bytes
|`file.pdf%00.jpg`
|rejected (the null byte causes `DecodingStage` to throw; input is never stripped or repaired)
...
* Null byte rejection (the stage *throws* on null bytes; it never strips or "repairs" input — consistent with the no-repair rule in SEC-6)
...
NOTE: The `de.cuioss.http.security` core does *not* implement logging. There is no `CuiLogger`
usage anywhere in the package. Detected violations are surfaced as `UrlSecurityException` and
counted in memory by link:../../../cui-http-core/src/main/java/de/cuioss/http/security/monitoring/SecurityEventCounter.java[SecurityEventCounter]
(counting only — no log emission). Applications should log security events at the trust
boundary where the exception is caught.
