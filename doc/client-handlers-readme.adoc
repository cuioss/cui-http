...
through. A same-origin hop forwards them under either strategy. Note that `CredentialForwarding`
changes only *what is carried* across a hop - it never changes the refusal verdict. A hop the policy
refuses is not taken at all, so no credential is ever forwarded to it.
