= HTTP Result Pattern: Modern Result Handling
:toc: left
:toclevels: 3
...
interface HttpResult<T> {
    ...
    // Content access
    Optional<T> getContent();           // Present for Success, Optional for Failure
...
* **Don't assume content is always present** - even Success can have Optional.empty() in edge cases
