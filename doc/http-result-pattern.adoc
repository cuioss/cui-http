...
[source,java]
----
HttpResult<ConfigData> result = adapter.getBlocking();
...
if (result.isSuccess()) {
    ...
    result.getETag().ifPresent(etag ->
        logger.debug("Content ETag: %s", etag));
...
result.getErrorCategory().ifPresent(category -> {
    switch (category) {
        ...
        case INTERRUPTED_ERROR -> {
            // The caller asked us to stop - never retry, the flag is already restored
...
interface HttpResult<T> {
    ...
    // Content access
    Optional<T> getContent();           // Present for Success with content; empty for Void/status-only Success or for Failure without fallback
...
HttpResult<String> jsonResult = adapter.getBlocking();
...
public class GenericHttpLoader<T> {
    ...
    public CompletableFuture<Boolean> loadData() {
        return CompletableFuture.supplyAsync(() -> {
            HttpResult<T> result = resilientAdapter.getBlocking();

            return switch (result) {
                case HttpResult.Success<T>(var data, var etag, var status) -> {
                    updateCache(data);
...
public class CachedHttpResolver<T> {
    ...
    private Optional<T> ensureLoaded() {
        if (cachedResult == null) {
            cachedResult = httpAdapter.getBlocking();
        }
