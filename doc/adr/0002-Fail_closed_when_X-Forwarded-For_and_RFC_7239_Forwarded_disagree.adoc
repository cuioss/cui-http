...
Superseded by xref:0019-Unresolvable_Forwarded_header_suppresses_only_the_fields_it_carried.adoc[ADR-0019:
Unresolvable Forwarded header suppresses only the fields it carried].

**What survives.** The disagreement rule itself is retained unchanged: for each field
the de-facto and RFC 7239 sources are resolved independently, and when both are
present and disagree the field resolves empty rather than letting either source win
by precedence. So is the reasoning behind it — a fixed precedence in either
direction hands an attacker a known-winning header to write.

**What is replaced.** Only the treatment of an unresolvable header as *present for
every field it could theoretically have carried*. Presence is now decided per field,
by the directives the parser actually reached, so a header that broke before a given
directive suppresses nothing for that field. The blanket scope was a
client-triggerable denial of service on the trust model: appending one broken
directive erased everything a legitimate proxy attested. A field the header *did*
speak about still fails closed, exactly as this ADR requires.

ADR-0019 also settles two questions this ADR did not reach — reconciling the two
de-facto families (`X-Forwarded-*` against `X-Proxy*`) against each other before the
RFC 7239 comparison, and comparing host and port as independent fields.
...
* xref:0019-Unresolvable_Forwarded_header_suppresses_only_the_fields_it_carried.adoc[ADR-0019: Unresolvable Forwarded header suppresses only the fields it carried] — supersedes this ADR
