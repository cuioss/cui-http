...
. *sanitizes* it through the header-value pipeline (rejecting over-length input, NUL, other
  control characters, and — when so configured — extended ASCII),
. applies field-specific *normalization* and injection guards, and
. *honors* it only when the configured trust model permits.

A value that fails sanitization is dropped and logged (`HTTP-122`). A value that is merely *not
trusted* is also dropped, but only the context-path drop is logged, and at `DEBUG`; scheme, host,
port, and client IP are discarded without a log line.

`resolve` is fail-safe for request-time use in the sense that no rejected header value produces an
exception — but it is not exception-free. It throws `NullPointerException` for a `null`
`headerLookup`, propagates any `RuntimeException` thrown by the caller-supplied lookup, and
propagates any non-`UrlSecurityException` escaping the sanitization pipeline.
...
`HTTP-120`..`HTTP-127` WARN records for rejected/malformed values (context-path control
characters, protocol-relative/backslash prefixes, values that failed sanitization, unparseable
client-IP chain entries, — `HTTP-124` — a field *dropped* because the de-facto family and the
RFC 7239 `Forwarded` header disagree — `HTTP-125` (`FORWARDED_DIRECTIVE_MALFORMED`) — a `Forwarded`
header carrying a malformed `forwarded-pair` — `HTTP-126` — a blank `trustedProxies` entry — and
`HTTP-127` (`DE_FACTO_FAMILIES_DISAGREE`) — the two de-facto families disagreed and the
`deFactoPrecedence` knob broke the tie, so the field *survived*).

== Header precedence

Each source is resolved *independently* and the results are then reconciled — the two de-facto
families (`X-Forwarded-*` and `X-Proxy*`) against each other first, and that result against RFC 7239
`Forwarded`. Only the context path keeps a first-present order, because its two names are not a
de-facto family pair. A disagreement between the two de-facto families is broken by the
`deFactoPrecedence` knob (default `X_FORWARDED`) and logged; a disagreement against RFC 7239 drops
the field.
...
| scheme      | `X-Forwarded-Proto` reconciled with `X-ProxyScheme`, then against RFC 7239 `proto`
| host        | `X-Forwarded-Host` reconciled with `X-ProxyHost`, then against RFC 7239 `host` — the host alone, not the `host[:port]` token as a whole
| port        | `X-Forwarded-Port` reconciled with `X-ProxyPort`, then against the port the RFC 7239 `host` directive carried; when no explicit port header is present at all, the port carried by the resolved host (`host:port`) is the fallback
| contextPath | `X-ProxyContextPath` → `X-Forwarded-Prefix`, first present (RFC 7239 has no prefix directive)
| clientIp    | `X-Forwarded-For` chain reconciled against the RFC 7239 `for` chain
|===
...
*Conflicting sources = drop (fail closed).* For scheme, host, port, and client IP the de-facto
...
*Host and port are compared as separate fields, and silence is not a conflict.* A source states both
in one `host[:port]` token, but the token is compared field by field: a host disagreement drops only
the host, a port disagreement only the port. A source whose token carried *no* port says nothing
about the port and therefore contests none — only two sources that both name a port, differently,
are in conflict.
...
X-Forwarded-Port: 8443 + Forwarded: host="h:8443"  -> port   = 8443   (explicit port agrees with the RFC host port)
X-Forwarded-Port: 8443 + Forwarded: host="h:9000"  -> port   = empty  (explicit port is reconciled, not honored unopposed)
X-Forwarded-Port: 8443 + Forwarded: host=h         -> port   = 8443   (the RFC host named no port, so it contests none)
----
...
*A malformed `Forwarded` value makes the header unbelievable — for the fields it actually spoke
about.* When its raw value fails sanitization, *or* when it carries a malformed `forwarded-pair`, the
header contributes *nothing*: every field it did carry a directive for meets an empty RFC resolution
and is dropped through the ordinary disagreement path above, rather than letting the de-facto family
win by default. Scope is *per field*, decided by the directives the parser reached before it
stopped — `Forwarded: proto=http;broken` drops the scheme but leaves an `X-Forwarded-Host` value
standing, because that header never spoke about the host. Suppressing every field on any garbage
input was a denial of service on the trust model: an attacker appended one broken directive and
erased everything a legitimate proxy attested. A pair is malformed when it is non-blank and carries
no `=`, an empty directive name, or an empty value. Rejection is logged once as an `HTTP-125` warning
with a log-sanitized value.
...
Forwarded: host                                        -> no field dropped     (no "=" -> malformed before any directive was reached)
X-Forwarded-Proto: https + Forwarded: proto=;host       -> scheme = https       (the empty value broke the parse before "proto" was recorded)
X-Forwarded-Proto: https + Forwarded: proto=http;broken -> scheme = empty       (the header DID speak about proto, so it fails closed)
X-Forwarded-Host: app.internal + Forwarded: proto=http;broken -> host = app.internal (it never spoke about the host)
X-Forwarded-Port: 8080 + Forwarded: host=t.ex;bad       -> port = 8080          (the host directive named no port, so it contests none)
X-Forwarded-Port: 8080 + Forwarded: host=t.ex:9443;bad  -> port = empty         (it did name one, so the port falls with the header)
----
...
| `deFactoPrecedence`
| Tie-breaker consulted *only* when the two de-facto families disagree on a field:
  `X_FORWARDED` (default) or `X_PROXY`. Set it to the family this deployment's own ingress writes;
  it presumes the ingress strips the family it does not write.
|===
...
* Sanitization happens *once, on the whole header value*, before the chain is split: the raw
  `X-Forwarded-For` value goes through the header-value pipeline as a single string and is only then
  split on commas. (The RFC 7239 `for` chain is covered the same way — the whole `Forwarded` value
  is sanitized once in `parseForwarded` before its directives are parsed.) Each resulting entry is
  then parsed as an IP literal (optional `:port` stripped, bracketed IPv6 unwrapped). Parsing is
  literal-only and never performs DNS resolution.
...
* Every honored value first passes through the header-value pipeline
  (xref:security-readme.adoc[Security Validation Framework]), which rejects over-length input, NUL,
  other control characters (CR/LF among them), and — when so configured — extended ASCII.
...
`ForwardedResolverConfig` (default `SecurityConfiguration.defaults()`), but only through the four
xref:security-readme.adoc[`SecurityConfiguration`] knobs the pipeline actually consumes:

* `maxHeaderValueLength` — enforced by `LengthValidationStage`.
* `allowNullBytes`, `allowControlCharacters`, `allowExtendedAscii` — enforced by
  `CharacterValidationStage`.

Every other knob has *no effect* on forwarded sanitization. `PipelineFactory.createHeaderValuePipeline`
builds a `HEADER_VALUE` pipeline of `LengthValidationStage` + `CharacterValidationStage` only — no
`DecodingStage`, no `NormalizationStage`, no `PatternMatchingStage`, and `AllowBlockListStage` is
added for `HEADER_NAME` only. So `normalizeUnicode`, `allowDoubleEncoding`,
`failOnSuspiciousPatterns`, and the header allow/block lists never run against a forwarded header
value, and `SecurityConfiguration.strict()` / `.lenient()` change forwarded behaviour only through
the four knobs above.
