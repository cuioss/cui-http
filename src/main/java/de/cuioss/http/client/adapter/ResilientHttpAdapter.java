        ...
        return operation.get()
                .thenCompose(result -> {
                    ...
                    if (config.idempotentOnly() && !method.isIdempotent()) {
                        /*~~(TODO: WARN needs LogRecord. Suppress: // cui-rewrite:disable CuiLogRecordPatternRecipe)~~>*/LOGGER.warn("Skipping retry for non-idempotent method: %s (idempotentOnly=true)",
                                method.methodName());
                        return CompletableFuture.completedFuture(result);
                    }
                    ...
                    if (attempt >= config.maxAttempts()) {
                        /*~~(TODO: WARN needs LogRecord. Suppress: // cui-rewrite:disable CuiLogRecordPatternRecipe)~~>*/LOGGER.warn("%s request failed after %s attempts", method.methodName(), config.maxAttempts());
                        return CompletableFuture.completedFuture(result);
                    }
                    ...
                    /*~~(TODO: WARN needs LogRecord. Suppress: // cui-rewrite:disable CuiLogRecordPatternRecipe)~~>*/LOGGER.warn("%s request failed on attempt %s, retrying after %sms",
