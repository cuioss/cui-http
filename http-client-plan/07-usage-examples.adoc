...
== Example 14: Clear ETag Cache (Rarely Needed)

Manual cache clearing is rarely needed with automatic eviction. Use only for:
...
// Clear cache on user logout (valid use case)
void onUserLogout() {
    adapter.clearETagCache();  // Remove user-specific cached data
}

// ❌ NOT needed for memory management - automatic eviction handles this
// ❌ NOT needed for periodic maintenance - cache self-manages
----
