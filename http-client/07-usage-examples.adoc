...
// Bidirectional content converter for User objects
...
// Send POST request (bidirectional converter handles serialization)
...
HttpAdapter<User> adapter = ETagAwareHttpAdapter.<User>builder()
    .httpHandler(handler)
    .contentConverter(userConverter)
    .build();  // Default: CacheKeyHeaders.ALL
...
//   ✅ Solution: Use CacheKeyHeaders.NONE (see Example 6b below)
...
// Use
HttpResult<User> result = resilientAdapter.get();
...
HttpContentConverter<String> xmlConverter = new StringContentConverter<>() {
    @Override
...
[source,java]
----
// Adapter with String bidirectional converter
HttpContentConverter<String> textConverter = new StringContentConverter<>() {
    @Override
    protected Optional<String> convertString(String rawContent) {
        return Optional.of(rawContent);
    }
...
HttpAdapter<String> textAdapter = ETagAwareHttpAdapter.<String>builder()
    .httpHandler(handler)
    .contentConverter(textConverter)
    .build();
...
// Binary converter
HttpContentConverter<byte[]> imageConverter = new HttpContentConverter<>() {
    @Override
    ...
    @Override
    public HttpResponse.BodyHandler<byte[]> getBodyHandler() {
